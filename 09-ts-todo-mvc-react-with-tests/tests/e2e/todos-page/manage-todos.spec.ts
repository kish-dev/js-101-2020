...
function interceptEmptyTodoList() {
    cy.intercept(
        'GET',
        todosEndpoint,
...
function interceptTodoListWithOneTask() {
    cy.intercept(
        'GET',
        todosEndpoint,
...
function authorize(intercept: () => void) {
    cy.intercept(
        'GET',
        getUserEndpoint,
...
describe('Manage Todos', () => {
    context('Creation', () => {
        it('Create todo', () => {
            ...
            authorize(interceptTodoListWithOneTask);
...
describe('Delete Todos', () => {
    context('Delete', () => {
        it('Delete Todo', () => {
            cy.fixture('single-todos-list.response.json').then((todoItemDelete) => {
                cy.intercept(
                    'DELETE',
                    todosEndpoint,
                    req => {
